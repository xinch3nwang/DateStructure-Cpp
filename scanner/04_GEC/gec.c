...
void SYSTEM_FUNCTION(uint8_t * data)
{
    ...
    switch(data[0])
    {
        ...
        {
            switch(strncmp((char *)gcReccrc32,(char *)gcRecvBuf+1+17,4))
            {
                case 0:
                  //emuart_sendN(2,(uint8_t *)"ok");
                  //break;//不可使用return，会造成串口丢失。【LQ20190918】
                default:
                ...
                else
                {
                    ...
                    gcRecvDCLen=0;
                    gcReccrc32[0]=gcRecvBuf[1+17];
                    gcReccrc32[1]=gcRecvBuf[2+17];
                    gcReccrc32[2]=gcRecvBuf[3+17];
                    gcReccrc32[3]=gcRecvBuf[4+17];
                    break;
                }
                break;
            }
